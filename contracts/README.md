...
Derive `ink::storage::traits::StorageLayout` (for `std` feature) on types that you want to store in `Mapping` map. See [`Bulletin`](./bulletin_board/lib.rs#L99)struct and its [usage](./bulletin_board/lib.rs#116) in the `Mapping` of the `BulletinBoard` contract state.
...
If you want your method to accept token transfer, you need to tag it with `payable` keyword: `#[ink(payable)]`. See [`BulletinBoard::post`](./bulletin_board/lib.rs#215) for an example.

For an example on how to do a cross-contract call _and_ transfer tokens, see [`highlight_post`](./bulletin_board/lib.rs#446).

### Unit test a contract

See [`bulletin_board::tests`](./bulletin_board/lib.rs#515) for various tests, including mocking the blockchain environment - setting caller, token balance, etc.
